...
from ros_x_habitat_interfaces.srv import ResetAgent, GetAgentTime
...
#             or self.agent_config.INPUT_TYPE == "rgbd"
#         ):
#             # you might need to replace this by sensor_msgs/msg/PointCloud2
...
class HabitatAgentNode(Node):
    ...
    def __init__(self) -> None:
        ...
        # set up logger
        self.logger = utils_logging.setup_logger('habitat_agent')
        self.initialize_threading_utils()
        self.initialize_agent()
        self.initialize_services()
        self.intialize_topics()
        self.logger.info("agent initialized")

    def initialize_threading_utils(self) -> None:
        self.get_logger().info('Initailizing threading utils...')
        self.lock = Lock()
        # shutdown triggers the node to be shutdown. Guarded by shutdown_cv
        self.shutdown_cv = Condition()
        with self.shutdown_cv:
    ...
    def initialize_services(self) -> None:
        self.get_logger().info('Initailizing services...')
        # establish agent reset service server
        self.reset_service = self.create_service(
            ResetAgent, ServiceNames.RESET_AGENT,
            self.reset_agent)

        # establish agent time service server
        self.agent_time_service = self.create_service(
            GetAgentTime, ServiceNames.GET_AGENT_TIME,
            self.get_agent_time)
        # self.agent_time_service = rospy.Service(
        #     f"{PACKAGE_NAME}/{self.node_name}/{ServiceNames.GET_AGENT_TIME}",
        #     GetAgentTime,
    ...
    def intialize_topics(self) -> None:
        self.get_logger().info('Initailizing topics...')
        # agent publish and subscribe queue size
        # TODO: make them configurable by constructor argument
        self.sub_queue_size = 10
        self.pub_queue_size = 10
        ...
        # "action", Int16, queue_size=self.pub_queue_size)
        self.pub = self.create_publisher(
            Int16, "action", self.pub_queue_size)
        ...
        if (
            ...
            self.sub_rgb = message_filters.Subscriber(self, Image, "rgb")
        if (
            self.agent_config.INPUT_TYPE == "depth"
            or self.agent_config.INPUT_TYPE == "rgbd"
        ):
        ...
        self.sub_pointgoal_with_gps_compass = message_filters.Subscriber(
            self, PointGoalWithGPSCompass, "pointgoal_with_gps_compass"
        )
...
#             pass
    ...
    def reset_agent(self, request):
        ...
        if request.reset == AgentResetCommands.RESET:
            ...
            with self.lock:
                ...
                self.agent_config.RANDOM_SEED = request.seed
                OmegaConf.set_readonly(self.agent_config, False)
                self.agent = PPOAgent(self.agent_config)
                OmegaConf.set_readonly(self.agent_config, True)
                self.agent.reset()
            return True
        elif request.reset == AgentResetCommands.SHUTDOWN:
            # shut down the agent node
    ...
    def msgs_to_obs(
        ...
        if rgb_msg is not None:
            observations["rgb"] = (
                CvBridge().imgmsg_to_cv2(rgb_msg, "passthrough").
                astype(np.float32)
            )
        ...
        if depth_msg is not None:
            observations["depth"] = self.depthmsg_to_cv2(depth_msg)
            # have to manually add channel info
            observations["depth"] = np.expand_dims(observations["depth"], 2).\
                astype(np.float32)
