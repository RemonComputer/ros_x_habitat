...
class GazeboToHabitatAgent(Node):
    def __init__(self) -> None:
        ...
        # Adding the publisher and subscribers queue sizes
        self.sub_queue_size = 1
        self.pub_queue_size = 1

        # set up logger
        self.logger = utils_logging.setup_logger('gazebo_to_habitat_agent')
        # set max number of steps for navigation
        # TODO: make them configurable by constructor argument
        self.max_steps = 500

        self.initialize_threading_locks()
        self.initialize_topics()
        self.get_logger().info('Node executed successfully...')

    def initialize_threading_locks(self) -> None:
        # bridge's publish and subscribe queue size
        ...
        self.curr_pose_lock = Lock()
        with self.curr_pose_lock:
            # NOTE: self.prev_pos is for visualization only
            self.prev_pos = None
            self.curr_pos = None
            self.curr_rotation = None
    ...
    def initialize_topics(self) -> None:
        fetch_goal_from_move_base_val = \
            self.get_parameter('fetch_goal_from_move_base').\
            get_parameter_value().bool_value
        final_pointgoal_pos_val = np.array(
            self.get_parameter('final_pointgoal_pos').
            get_parameter_value().double_array_value)
        if fetch_goal_from_move_base_val:
            # subscribe from goal position topic and get
            # the point-goal from there
            # self.sub_goal = rospy.Subscriber(
            #     move_base_goal_topic_name,
            ...
            # get goal directly from constructor argument
            with self.final_pointgoal_lock:
                self.final_pointgoal_pos = final_pointgoal_pos_val
                self.logger.info("Final pointgoal position set to: "
                                 f"{self.final_pointgoal_pos}")
                self.pointgoal_set = True
...
#             queue_size=self.sub_queue_size
#         )
        self.sub_last_action_done = self.create_subscription(
                Int16, 'last_action_done',
                self.callback_signal_last_action,
                self.sub_queue_size)
...
#             queue_size=self.pub_queue_size
#         )
        self.pub_init_and_goal_pos = self.create_publisher(
        ...
        #     self.get_agent_pose,
        # )
        self.get_agent_pose_service = self.create_service(
            GetAgentPose, f'{ServiceNames.GET_AGENT_POSE}',
            self.get_agent_pose)
            ...
            with self.pointgoal_reached_lock:
                with self.final_pointgoal_lock:
                    if (
                        ...
                        # create RGB message for Habitat
                        rgb_img = self.rgb_msg_to_img(rgb_msg, 256)
                        rgb_msg_for_hab = CvBridge().cv2_to_imgmsg(
                            rgb_img, encoding="rgb8")
                        rgb_msg_for_hab.header = h
                        # create depth message for Habitat
                        depth_img = self.depth_msg_to_img(depth_msg, 256)
                        depth_msg_for_hab = DepthImage()
                        depth_msg_for_hab.height, depth_msg_for_hab.width = \
                            depth_img.shape
                        depth_msg_for_hab.step = depth_msg_for_hab.width
                        depth_msg_for_hab.data = np.ravel(depth_img)
                        depth_msg_for_hab.header = h
                        ...
                        with self.curr_pose_lock:
                            if self.count_steps == 0:
                                self.logger.info(
                                    f"initial agent position: {self.curr_pos}")
                                self.add_pos_to_marker_array(
                                    "init",
                                    self.curr_pos
                                )
                        ...
                        # for visualization
                        self.logger.info(
                            f"navigation completed after {self.count_steps}"
                            " steps")
                        with self.curr_pose_lock:
                            self.logger.info(
                                f"final agent position: {self.curr_pos}")
                            self.add_pos_to_marker_array(
                                "goal",
                                self.final_pointgoal_pos
                            )
    ...
    def callback_register_goal(self, goal_msg):
        ...
        with self.final_pointgoal_lock:
            ...
            self.pointgoal_set = True
            self.logger.info(
                f"Final pointgoal position set to: {self.final_pointgoal_pos}")
