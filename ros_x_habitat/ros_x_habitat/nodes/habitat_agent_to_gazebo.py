...
class HabitatAgentToGazebo(Node):
    r"""
    ...
    def __init__(self,):
        super().__init__('habitat_agent_to_gazebo')
        # Declaring the parameter
        control_period_descriptor = ParameterDescriptor(
            description='time it takes for a discrete action to complete, '
            'measured in seconds')
        self.declare_parameter('control_period',
                               1.0,
                               control_period_descriptor)
        ...
        # set up logger
        self.logger = utils_logging.setup_logger('habitat_agent_to_gazebo')

        # register control period
        control_period_value = self.get_parameter('control_period').\
            get_parameter_value().double_value
        self.control_period = control_period_value
...
#         )
        self.sub_action = self.create_subscription(
            Int16, "action", self.callback_action_from_agent,
            self.pub_queue_size)
...
#         self.pub_vel = rospy.Publisher("cmd_vel", Twist, queue_size=self.pub_queue_size)
        self.pub_vel = self.create_publisher(
            Twist, "cmd_vel", self.pub_queue_size)
...
#         )
        self.pub_last_action_done = self.create_publisher(
            Int16, "last_action_done", self.pub_queue_size)
...
#         )
        self.get_agent_pose = self.create_client(
            GetAgentPose, ServiceNames.GET_AGENT_POSE)
    ...
    def callback_action_from_agent(self, action_msg):
        ...
        if action_id == _DefaultHabitatSimActions.STOP.value:
            ...
            while not self.get_agent_pose.wait_for_service(timeout_sec=0.1):
                self.get_logger().info('waiting for agent pose service...')
            try:
        ...
        # Sleep for for the control period
        control_period_value = self.get_parameter('control_period').\
            get_parameter_value().double_value
        rate = self.create_rate(1 / control_period_value)
        rate.sleep()
...
def main(args=None):
    ...
    habitat_agent_to_gazebo = HabitatAgentToGazebo()
    rclpy.spin(habitat_agent_to_gazebo)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    habitat_agent_to_gazebo.destroy_node()
    rclpy.shutdown()
