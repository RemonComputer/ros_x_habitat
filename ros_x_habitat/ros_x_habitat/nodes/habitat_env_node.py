...
class HabitatEnvNode(Node):
    def __init__(self):
        ...
        pub_rate_value = self.get_parameter('sensor_pub_rate').\
            get_parameter_value().double_value
        self.reset()
        self.simulation_timer_ = self.create_timer(
    ...
    def setup_publishers_and_subscriber(self) -> None:
        ...
        # we create one topic for each of RGB, Depth and GPS+Compass
        # sensor
        if "rgb_sensor" in self.config.habitat.simulator.agents.main_agent.sim_sensors:
            # self.pub_rgb = rospy.Publisher("rgb", Image, queue_size=self.pub_queue_size)
            self.pub_rgb = self.create_publisher(Image, 'rgb',
                                                 self.pub_queue_size)
        if "depth_sensor" in self.config.habitat.simulator.agents.main_agent.sim_sensors:
            if self.use_continuous_agent:
                # if we are using a ROS-based agent, we publish depth images
                # in type Image
                # self.pub_depth = rospy.Publisher(
